...
class IntegrationAgent(RoutedAgent):
    ...
    def _create_integration_prompt(
        ...
        # Filter classifier predictions with high confidence
        high_confidence_predictions = [
            pred for pred in classifier_verification.classifier_predictions
            if pred.confidence > 0.7
        ]

        # Build classifier prediction summary text
        classifier_summary = "\n".join([
        ...
        {{
            "merged_analysis": {{
                ...
                "refined_response": "the modified response aligned with classifier predictions"
            }}
        }}
        """
    ...
    async def _try_process_results(self, session_id: str) -> None:
        ...
        if not (
                'consistency' in results and 'verification' in results and 'classifier' in results and 'analysis_task' in results):
            print(f"Missing required results for session {session_id}")
            print(f"Current results: {list(results.keys())}")
            return
        ...
        try:
            ...
            model_reliabilities = {
                "mllm1": ModelReliability(
                    ...
                    classifier_score=classifier_result.mllm1_result.classifier_verification_score,
                    final_score=self._calculator.calculate_final_score(
                        ...
                        classifier_result.mllm1_result.classifier_verification_score
                    ),
                ...
                "mllm2": ModelReliability(
                    ...
                    classifier_score=classifier_result.mllm2_result.classifier_verification_score,
                    final_score=self._calculator.calculate_final_score(
                        verification_result.mllm2_validation.score,
                        consistency_result.mllm2_analysis.consistency_score,
                        classifier_result.mllm2_result.classifier_verification_score
                    ),
                ...
                "mllm3": ModelReliability(
                    ...
                    classifier_score=classifier_result.mllm3_result.classifier_verification_score,
                    final_score=self._calculator.calculate_final_score(
                        verification_result.mllm3_validation.score,
                        consistency_result.mllm3_analysis.consistency_score,
                        classifier_result.mllm3_result.classifier_verification_score
                    ),
            ...
            if best_model == 'mllm3':
                original_response = analysis_task.mllm3_response if analysis_task.mllm3_response else analysis_task.mllm4_response
            else:
            ...
            enhanced_response = await self._enhance_with_verification(
                refined_response,
                {
                    ...
                    'mllm3': analysis_task.mllm3_response if analysis_task.mllm3_response else analysis_task.mllm4_response
                },
                best_model,
                self._model_clients
            )

            # enhanced_response_result = await self._model_client.create(
            #     messages=[
            #         self._system_message,
    ...
    async def handle_consistency_result(self, message: ConsistencyResult, ctx: MessageContext) -> None:
        ...
        if session_id not in self._pending_results:
            self._pending_results[session_id] = {'analysis_task': None}
