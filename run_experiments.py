...
# (task_name, questions_file, mllm3_answers_file)
# questions_file  → questions/          {"question_id", "prompt"}
# mllm3_answers_file → MLLMs_output/   {"question_id", "text"}
ALL_TASKS = [
...
async def main():
    ...
    if "--dry-run" in args:
        ...
        print("-" * 72)
        for name, qf, af in ALL_TASKS:
            Config.configure_task(name, qf, af)
